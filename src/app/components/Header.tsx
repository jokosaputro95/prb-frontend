...
const navLinks = [
    { label: "Home", href: "#" },
    { label: "Read History", href: "#" },
    { label: "Economy", href: "#" },
    { label: "Travel", href: "#" },
    { label: "Global Security", href: "#" },
        ...
        <header className="sticky top-0 z-50 bg-blue-900 text-white">
            ...
            <div className="flex items-center justify-between px-3 sm:px-6 py-2">
                <div className="flex items-center gap-2">
                    <img src="/logo.svg" alt="Foxiz Logo" className="h-9 w-auto" />
                </div>
                {/* Desktop Menu */}
                ...
                <div className="hidden lg:flex items-center gap-2">
                    <button
                        ...
                        aria-label="Search"
                    >
                        <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="10" cy="10" r="7" /><path d="m15 15 4 4" /></svg>
                    </button>
                    <button className="hover:bg-blue-800 p-2 rounded-full" aria-label="Darkmode">
            ...
            {/* Mobile Menu Drawer */}
            <div
                className={`fixed inset-0 z-[100] bg-blue-900 text-white transition-all duration-200 ease-in-out ${mobileMenu ? "translate-x-0" : "translate-x-full"
                    }`}
            ...
            >
                ...
                <nav className="flex flex-col px-4 py-3 gap-3 text-xl font-semibold">
                    {navLinks.map((link) => (
                        <a key={link.label} href={link.href} className="py-2 border-b border-blue-800">{link.label}</a>
                    ))}
                </nav>
                <div className="flex gap-4 p-4">
                    <button className="flex-1 bg-yellow-400 text-black py-2 rounded font-bold">Newsletter</button>
                    <button className="bg-blue-800 p-2 rounded-full" aria-label="Search" onClick={() => {
                        setSearchOpen(true); setMobileMenu(false);
                    }}>
                        <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="10" cy="10" r="7" /><path d="m15 15 4 4" /></svg>
                    </button>
                </div>
            ...
            {/* Search Drawer */}
            <div
                className={`fixed inset-0 z-[100] bg-blue-900 text-white flex flex-col transition-all duration-200 ${searchOpen ? "translate-y-0" : "-translate-y-full"
                    }`}
            ...
            >
                ...
                <form className="flex items-center px-4 py-6">
                    <input
                        ...
                        autoFocus
                    />
                    <button className="bg-yellow-400 text-black px-4 py-3 rounded-r font-bold" type="submit">
                        Search
                    </button>
                </form>
            </div>
